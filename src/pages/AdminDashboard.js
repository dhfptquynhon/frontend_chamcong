...
const AdminDashboard = () => {
  ...
  const fetchOverviewStats = async () => {
    ...
    try {
      const res = await axios.get('http://localhost:5000/api/admin/overview-stats', {
        headers: { Authorization: `Bearer ${auth.token}` }
      });
      setStats(res.data);
    } catch (err) {
  ...
  const fetchAttendanceReport = async (month, year) => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:5000/api/admin/attendance-report?month=${month}&year=${year}`,
        { headers: { Authorization: `Bearer ${auth.token}` } }
      );
  ...
  const exportAttendanceReport = async () => {
    ...
    try {
      const url = `http://localhost:5000/api/admin/export/attendance-report?month=${reportDate.month}&year=${reportDate.year}`;
  ...
  const fetchPendingTrucThay = async () => {
    ...
    try {
      const res = await axios.get('http://localhost:5000/api/admin/pending-tructhay', {
        headers: { Authorization: `Bearer ${auth.token}` }
      });
  ...
  const fetchEmployeeDetail = async (employeeId) => {
    ...
    try {
      const [scheduleRes, attendanceRes, statsRes, trucThayRes] = await Promise.all([
        axios.get(`http://localhost:5000/api/admin/employee/${employeeId}/schedule?month=${reportDate.month}&year=${reportDate.year}`, {
          headers: { Authorization: `Bearer ${auth.token}` }
        }),
        axios.get(`http://localhost:5000/api/admin/employee/${employeeId}/attendance?month=${reportDate.month}&year=${reportDate.year}`, {
          headers: { Authorization: `Bearer ${auth.token}` }
        }),
        axios.get(`http://localhost:5000/api/admin/employee/${employeeId}/monthly-stats?month=${reportDate.month}&year=${reportDate.year}`, {
          headers: { Authorization: `Bearer ${auth.token}` }
        }),
        axios.get(`http://localhost:5000/api/admin/employee/${employeeId}/tructhay`, {
          headers: { Authorization: `Bearer ${auth.token}` }
        })
      ]);
  ...
  const exportSummaryReport = async () => {
    ...
    try {
      const url = `http://localhost:5000/api/admin/export/summary-report?month=${reportDate.month}&year=${reportDate.year}`;
  ...
  const handleSaveEmployee = async () => {
    ...
    try {
      ...
      if (mode === 'create') {
        await axios.post('http://localhost:5000/api/admin/employees/create', employee, {
          headers: { Authorization: `Bearer ${auth.token}` }
        });
      } else {
        ...
        await axios.put(`http://localhost:5000/api/admin/employees/${selectedEmployee?.id}`, updateData, {
          headers: { Authorization: `Bearer ${auth.token}` }
        });
      }
  ...
  const handleApproveTrucThay = async (trucThayId, approve) => {
    try {
      ...
      const res = await axios.post(
        `http://localhost:5000/api/admin/tructhay/${trucThayId}/approve`,
        { approve },
        { headers: { Authorization: `Bearer ${auth.token}` } }
      );
